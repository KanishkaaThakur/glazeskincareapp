...
const Navbar = () => {
    ...
    // Check if we are on the login page to simplify the UI
    const isLoginPage = location.pathname === '/login';

    return (
        <div className='flex items-center justify-between py-5 font-medium'>
            
            {/* Logo stays on all pages */}
            <Link to='/'>
                <img src={assets.logo} className='w-36' alt="Glaze Logo" />
            </Link>

            {/* Only show navigation and icons if NOT on the login page */}
            {!isLoginPage && (
                <>
            ...
            {/* Sidebar menu for small screens (also hidden on login) */}
            {!isLoginPage && (
